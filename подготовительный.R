...
Ortho.impulse <- function(m, dv, iv, times, Bo) {
  n  <- m[["n"]]
  lm <- m[["lm"]]
  ...
  o <- function(x , a) x%*%a
  z <- lapply(z, o, solve(l))
  
  imp <- c(solve(l)[dv, iv], rep(0, times))
  for (j in 1:times) {
    imp[1+j] <- z[[j]][dv, iv]
  }
  return(imp)
...
CI.fromboot <- function(m, R, FUN.oir, Bo.dec, dv, iv, times) {
  ...
  x<-apply(imp, 2, quantile, c(.01, .05, .01, .90, 0.95, .99))
  return(x)
}
#==== Данные ====
dataindex <- read.csv("index.csv", header = TRUE, sep = ";", dec = ".")
